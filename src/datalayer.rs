...
use crate::Error;
use crate::parser::housenumber::CountableHouseNumber;
  ...
  fn calculate_house_number_count(mut self, house_number_points: &Vec<HouseNumberPoint>) -> Self {
    ...
    for house_number in house_number_points {
        if self.polygon.contains(&house_number.point) {
            ...
            house_numbers.merge(HouseNumberList::try_from(house_number.text.as_str()).unwrap_or_default());
        }
    }
    self.tags.insert("addr:housenumber_count".to_string().into(), house_numbers.count().to_string().into());
    self
  }
}
...
impl Display for Building {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "Building {{ polygon: {:?}, tags: {:?} }}", self.polygon, self.tags)
  }
}
...
pub fn load_buildings(
  ...
  let osm_building_ways: Vec<Way> = osm_buildings.clone().into_iter().filter_map(|(_, obj)| match obj {
      OsmObj::Way(inner) => Some(inner),
        _ => None,
    }).collect();

  let osm_building_nodes: BTreeMap<NodeId, Node> = osm_buildings.into_iter().filter_map(|(_, obj)| match obj {
      OsmObj::Node(inner) => Some(inner),
        _ => None,
    })
    .map(|node| (node.id, node)).collect();

  let osm_housenumber_nodes: BTreeMap<NodeId, Node> = osm_housenumber.into_iter().filter_map(|(key, obj)| match obj {
    OsmObj::Node(inner) => Some(inner),
      _ => None,
  }).map(|node| (node.id, node)).collect(); // TODO: How to get rid of this map function?

  // Create geometry for buildings
  let mut buildings: Vec<Building> = osm_building_ways.into_iter().map(|obj| {
    let coords: Vec<(f64, f64)> = obj.nodes.iter().map(|node_id| (osm_building_nodes[node_id].decimicro_lat as f64 / 10000000., osm_building_nodes[node_id].decimicro_lon as f64 / 10000000.)).collect();
    let line_string = geo::LineString::from(coords);
    let polygon = Polygon::new(line_string, vec![]).convex_hull();
    Building{polygon: polygon, tags: obj.tags}
  }).collect();

  let housenumbers: Vec<HouseNumberPoint> = osm_housenumber_nodes.iter().map(|(_,obj)| {
    let point = Point::new(obj.decimicro_lat as f64 / 10000000., obj.decimicro_lon as f64 / 10000000.);
    let text = obj.tags["addr:housenumber"].to_string();
    HouseNumberPoint{point: point, text: text}
  }).collect();
  // Calculate house number count
  
  buildings = buildings.into_iter().map(|building| building.calculate_house_number_count(&housenumbers)).collect();
